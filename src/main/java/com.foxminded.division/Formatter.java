...
public class Formatter {
    ...
    private StringJoiner buildHead(CalculatorResult dto) {
        ...
        if (!dto.getStepList().get(0).isLast()) {
            divided = dto.getStepList().get(0).getDivided();
        }
    ...
    private StringJoiner buildBody(CalculatorResult dto, StringJoiner out) {
        ...
        for (var i = 1; i < dto.getStepList().size(); i++) {
            var divided = dto.getStepList().get(i - 1).getDivided();
            var division = dto.getStepList().get(i - 1).getDivision();
            ...
            if (dto.getStepList().get(i).isLast()) {
                ...
                out.add(addChar(offset, SPACE) + dto.getStepList().get(i).getDivision());
                if (String.valueOf(dto.getStepList().get(i).getDivision()).length() > String.valueOf(dto.getStepList().get(i).getDivided()).length()) {
                    offset++;
                }
                out.add(addChar(offset, SPACE) + dto.getStepList().get(i).getDivided());
                out.add(addChar(offset, SPACE) + addChar(String.valueOf(dto.getStepList().get(i).getDivided()).length(), DASH));
            }
        }
        return out;
    }
